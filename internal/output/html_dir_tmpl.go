...
nav { position: sticky; top: 0; z-index: 100; background: var(--card-bg); border-bottom: 1px solid var(--border); padding: .5rem 1rem; margin: -1rem -1rem 1.5rem; display: flex; gap: 1.5rem; font-size: .875rem; }
...
thead { position: sticky; top: 2.5rem; background: var(--card-bg); }
...
const htmlDirJS = `function svgEl(tag, attrs) {
...
const htmlDirPageTemplate = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<script>var chartData = {{json .ChartData}};</script>
