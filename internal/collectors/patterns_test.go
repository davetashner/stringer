...
func TestLargeFileDetected(t *testing.T) {
	...
	// Create a file with 1600 lines (above the 1500-line threshold).
	...
	assert.Contains(t, largeFileSignals[0].Title, "1600 lines")
...
func TestLargeFileNotDetectedUnderThreshold(t *testing.T) {
	...
	// Create a file with exactly 1500 lines (at threshold, not over).
	content := strings.Repeat("package main\n", 1500)
...
func TestLargeFileConfidenceScaling(t *testing.T) {
	tests := []struct {
		...
		{name: "just_over_default", lines: 1510, threshold: 1500, wantMinConf: 0.4, wantMaxConf: 0.45},
		{name: "1.5x_default", lines: 2250, threshold: 1500, wantMinConf: 0.55, wantMaxConf: 0.65},
		{name: "2x_default", lines: 3000, threshold: 1500, wantMinConf: 0.79, wantMaxConf: 0.80},
		{name: "3x_default_capped", lines: 4500, threshold: 1500, wantMinConf: 0.80, wantMaxConf: 0.80},
...
func TestLargeFileGeneratedFileExcluded(t *testing.T) {
	dir := t.TempDir()

	// Create a 2000-line file with a "Code generated" header — should be excluded.
	content := "// Code generated by protoc-gen-go. DO NOT EDIT.\n" + strings.Repeat("package main\n", 1999)
	require.NoError(t, os.WriteFile(filepath.Join(dir, "api.pb.go"), []byte(content), 0o600))
	...
	for _, s := range signals {
		if s.Kind == "large-file" {
			t.Errorf("unexpected large-file signal for generated file: %s", s.Title)
		}
	}
}

func TestLargeFileGoStringerExcluded(t *testing.T) {
	dir := t.TempDir()

	// Create a 2000-line *_string.go file (Go stringer output) — should be excluded.
	content := strings.Repeat("package main\n", 2000)
	require.NoError(t, os.WriteFile(filepath.Join(dir, "foo_string.go"), []byte(content), 0o600))
	...
	for _, s := range signals {
		if s.Kind == "large-file" {
			t.Errorf("unexpected large-file signal for stringer-generated file: %s", s.Title)
		}
	}
}
...
func TestIsGeneratedFile(t *testing.T) {
	...
	// Marker on line 3 (after a license header) → generated.
	lateHeader := filepath.Join(dir, "late.go")
	require.NoError(t, os.WriteFile(lateHeader, []byte("// Copyright\n// SPDX\n// DO NOT EDIT.\npackage main\n"), 0o600))
	assert.True(t, isGeneratedFile(lateHeader))

	// Marker beyond the header window → not generated.
	deepMarker := filepath.Join(dir, "deep.go")
	body := strings.Repeat("// comment\n", generatedHeaderLines) + "// Code generated late\n"
	require.NoError(t, os.WriteFile(deepMarker, []byte(body), 0o600))
	assert.False(t, isGeneratedFile(deepMarker))

	// Minified bundle → generated.
	minified := filepath.Join(dir, "bundle.js")
	require.NoError(t, os.WriteFile(minified, []byte(strings.Repeat("!function(e){e.x=1};", 30)+"\n"), 0o600))
	assert.True(t, isGeneratedFile(minified))

	// Large minified file whose first 16KB has no newline at all → generated.
	huge := filepath.Join(dir, "huge.js")
	require.NoError(t, os.WriteFile(huge, []byte(strings.Repeat("a", 3*classifyReadBytes)), 0o600))
	assert.True(t, isGeneratedFile(huge))

	// Unreadable path → not generated.
	assert.False(t, isGeneratedFile(filepath.Join(dir, "missing.go")))

	// Empty file → not generated.
	empty := filepath.Join(dir, "empty.go")
	require.NoError(t, os.WriteFile(empty, nil, 0o600))
	assert.False(t, isGeneratedFile(empty))
}
...
func TestHasGeneratedHeader(t *testing.T) {
	tests := []struct {
		...
		{"go_codegen", "// Code generated by mockery. DO NOT EDIT.\npackage x\n", true},
		{"do_not_edit_only", "# DO NOT EDIT - managed by terraform\n", true},
		{"at_generated", "/**\n * @generated\n */\n", true},
		{"dotnet_auto_generated", "//------------\n// <auto-generated>\n//   This code was generated by a tool.\n", true},
		{"marker_on_fifth_line", "1\n2\n3\n4\n// DO NOT EDIT\n", true},
		{"marker_on_sixth_line", "1\n2\n3\n4\n5\n// DO NOT EDIT\n", false},
		{"plain_source", "package main\n\nfunc main() {}\n", false},
		{"empty", "", false},
		{"lowercase_do_not_edit_ignored", "// do not edit this by hand please\n", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, hasGeneratedHeader([]byte(tt.content)))
		})
	}
}

func TestIsMinifiedContent(t *testing.T) {
	long := strings.Repeat("x", minifiedAvgLineLen+1)
	short := "short line"
	tests := []struct {
		name    string
		content string
		...
		{"empty", "", false},
		{"only_newlines", "\n\n\n", false},
		{"single_long_line_no_newline", long, true},
		{"single_long_line", long + "\n", true},
		{"exactly_threshold_not_minified", strings.Repeat("x", minifiedAvgLineLen) + "\n", false},
		{"normal_source", strings.Repeat(short+"\n", 40), false},
		{"long_lines_after_sample_window_ignored", strings.Repeat(short+"\n", minifiedSampleLines) + long + "\n", false},
		{"mixed_average_over_threshold", strings.Repeat("y", 1000) + "\n" + strings.Repeat("y", 1000) + "\n" + short + "\n", true},
		{"long_license_line_then_code", strings.Repeat("y", 1000) + "\n" + strings.Repeat(short+"\n", 19), false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, isMinifiedContent([]byte(tt.content)))
		})
	}
}
...
func TestLargeFileConfidence_AtThreshold(t *testing.T) {
	// At exactly the threshold+1 (1501), confidence should be just above 0.4.
	conf := largeFileConfidence(1501, defaultLargeFileThreshold)
	assert.GreaterOrEqual(t, conf, 0.4)
	assert.Less(t, conf, 0.41)
}

func TestLargeFileConfidence_ExtremelyLarge(t *testing.T) {
	// 10000 lines should still cap at 0.8.
