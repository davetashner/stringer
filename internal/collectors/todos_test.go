...
func TestShouldExclude(t *testing.T) {
	tests := []struct {
		...
		// Explicit any-depth prefix.
		{name: "doublestar_dir_root", relPath: "compiled/react.js", patterns: []string{"**/compiled/**"}, want: true},
		{name: "doublestar_dir_interior", relPath: "packages/next/src/compiled/react-dom/cjs/react-dom.production.js", patterns: []string{"**/compiled/**"}, want: true},
		{name: "doublestar_dir_itself", relPath: "packages/next/src/compiled", patterns: []string{"**/compiled/**"}, want: true},
		{name: "doublestar_dir_no_partial", relPath: "src/precompiled/a.js", patterns: []string{"**/compiled/**"}, want: false},
		{name: "doublestar_file_root", relPath: "app.min.js", patterns: []string{"**/*.min.js"}, want: true},
		{name: "doublestar_file_nested", relPath: "static/js/app.min.js", patterns: []string{"**/*.min.js"}, want: true},
		{name: "doublestar_file_no_match", relPath: "static/js/app.js", patterns: []string{"**/*.min.js"}, want: false},
...
func TestMergeExcludes_NegationOptsBackIn(t *testing.T) {
	got := mergeExcludes([]string{"!**/*.pb.go", "!not-a-default/**", "build/**"})

	assert.NotContains(t, got, "**/*.pb.go", "negated default must be removed")
	assert.NotContains(t, got, "!**/*.pb.go", "negation itself must not be passed through as a glob")
	assert.NotContains(t, got, "!not-a-default/**")
	assert.Contains(t, got, "build/**")
	assert.Contains(t, got, "**/compiled/**", "other defaults stay in place")
	assert.Len(t, got, len(defaultExcludePatterns))

	assert.False(t, shouldExclude("api/types.pb.go", got), "opted-in protobuf output must be scanned")
	assert.True(t, shouldExclude("api/types.pb.go", mergeExcludes(nil)), "default still excludes protobuf output")
}

func TestDefaultExcludes_CompiledAndGenerated(t *testing.T) {
	...
	excluded := []string{
		...
		"static/site.min.css",
		"dist/app.bundle.js",
		"lib/cjs/react.production.js",
		"lib/cjs/react.development.js",
		"pkg/apis/core/v1/zz_generated.deepcopy.go",
		"staging/src/k8s.io/api/core/v1/generated.pb.go",
		"internal/kind_string.go",
		"src/Models/Person.generated.cs",
		"src/api.generated.ts",
		"Client/Foo.g.cs",
		"Forms/MainForm.Designer.cs",
	}
	for _, p := range excluded {
		assert.True(t, shouldExclude(p, excludes), "%s should be excluded by default", p)
	}
	kept := []string{
		"src/production.js",
		"internal/collectors/todos.go",
		"pkg/apis/core/v1/types.go",
		"src/Models/Person.cs",
		"src/generated_report.md",
		"src/precompiled/index.js",
	}
	for _, p := range kept {
		assert.False(t, shouldExclude(p, excludes), "%s must not be excluded by default", p)
	}
}
...
func TestCollect_GeneratedContentSkipped(t *testing.T) {
	minified := "// TODO: minified\n" + strings.Repeat("var a=1;", 200) + "\n"
	repoPath := initTestGitRepo(t, map[string]string{
		"main.go":    "// TODO: keep this\n",
		"gen.go":     "// Code generated by protoc-gen-go. DO NOT EDIT.\npackage main\n// TODO: hidden\n",
		"schema.ts":  "/**\n * @generated SignedSource<<abc>>\n */\n// TODO: hidden\n",
		"Form.cs":    "// <auto-generated />\n// TODO: hidden\n",
		"bundle.out": minified,
	})
	...
	require.Len(t, signals, 1, "only main.go should produce a signal: %+v", signals)
	assert.Equal(t, "main.go", signals[0].FilePath)

	// An explicit include re-enables content-detected generated files.
	signals, err = c.Collect(context.Background(), repoPath, signal.CollectorOpts{
		IncludePatterns: []string{"*.go"},
	})
	require.NoError(t, err)
	...
	for _, sig := range signals {
		files[sig.FilePath] = true
	}
	assert.True(t, files["gen.go"], "explicit include should override generated-content skip")
