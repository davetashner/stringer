...
| `--exclude`             | `-e`  |         | Glob patterns to exclude from scanning; `!pattern` re-enables a default exclude (see below) |
...
**Default excludes.** All collectors skip `vendor/**`, `node_modules/**`,
`testdata/**`, `third_party/**`, `eval/**`, `.claude/worktrees/**`, precompiled
and minified bundles (`**/compiled/**`, `**/*.min.js`, `**/*.min.css`,
`**/*.bundle.js`, `**/*.production.js`, `**/*.development.js`) and generated
source (`**/zz_generated*.go`, `**/*.pb.go`, `**/*_string.go`,
`**/*.generated.*`, `**/*.g.cs`, `**/*.Designer.cs`). Files whose first five
lines contain `Code generated`, `DO NOT EDIT`, `@generated` or `<auto-generated`,
or whose first 20 lines average more than 400 characters, are also skipped.
Pass `-e '!**/*.pb.go'` (or the same string in `exclude_patterns`) to opt a
default glob back in. A pattern starting with `/` is anchored at the scanned
root: `-e /api/**` skips `api/` but not `internal/api/`. See [large-repos.md](large-repos.md#recommended-excludes).
