...
│   │   ├── gitlog.go           # Reverts, high-churn files, stale branches. The commit walk (reverts + 90-day churn maps) is memoised process-wide in gitlogHistories keyed by (git root, HEAD, depth, since) so a monorepo scan walks history once and every workspace reuses it; stale-branch detection stays per call. Signals are repo-root-relative and repo-wide; the scan/report workspace loop stamps the workspace prefix (stringer-jfh.5)
